...
public class Main {
    ...
    // 2차원 배열을 복사할 수 있다. 
		...
		// 상어 이동에 대해 dfs를 지정하려면,
		// 물고기 관련된 정보들을 복원해야 한다. 
		// 이 지점이 문제의 key point이다. 
    ...
    // 물고기 정보를 옮길 때는, 그냥 복사를 하면 안되고, 새로 생성해줘야 한다 
		for(int i=1; i<=16; i++)
	...
	public static void moveFish() {
		for(int i = 1; i < 17; i++) { //i는 현재 물고기의 번호 
			...
			// dir, cnt 변수를 구분해서 쓸 수 있다.
			// dir은 0~7로 나타내도록 할 수 있다. 
			while(cnt < 8) { //이동할 수 있는 위치를 찾을때까지 45도 방향 바꾸며 반복 
				...
				if(nx >= 0 && nx < 4 && ny >= 0 && ny < 4 && map[nx][ny] != -1) { 
					if(map[nx][ny] == 0) { //이동할 위치가 빈칸일 경우 
						...
						// 바꿀 물고기의 위치 먼저 이동시켜 준다
						// 그리고 map에 도 마찬가지로 정보를 업데이트 한다. 
